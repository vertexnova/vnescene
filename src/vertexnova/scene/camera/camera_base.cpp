...
std::pair<Vec3f, float> CameraBase::resolveBackUnitAndLookDistance(const Vec3f& eye,
                                                                   const Vec3f& target,
                                                                   const Quatf& orientation_fallback) noexcept {
    Vec3f back = eye - target;
...
void CameraBase::setTargetImpl(const Vec3f& target) noexcept {
    ...
    if (d > kEps) {
        ...
        // Eye and target coincide (or nearly): derived getTarget() must match — use zero distance.
        look_distance_ = 0.0f;
...
void CameraBase::lookAtImpl(const Vec3f& position, const Vec3f& target, const Vec3f& up) noexcept {
    position_ = position;
    up_hint_ = up;
    const auto back_and_d = resolveBackUnitAndLookDistance(position, target, orientation_);
    look_distance_ = back_and_d.second;
    orientation_ = orientationFromPosBack(back_and_d.first, up_hint_, Vec3f(0.0f, 1.0f, 0.0f));
    view_matrix_dirty_ = true;
}

void CameraBase::lookAtImpl(const Vec3f& target, const Vec3f& up) noexcept {
    up_hint_ = up;
    const auto back_and_d = resolveBackUnitAndLookDistance(position_, target, orientation_);
    look_distance_ = back_and_d.second;
    orientation_ = orientationFromPosBack(back_and_d.first, up_hint_, Vec3f(0.0f, 1.0f, 0.0f));
    view_matrix_dirty_ = true;
}
