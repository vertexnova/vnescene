...
#==============================================================================
# VneScene Documentation Generation Script
...
check_prerequisites() {
    ...
    if [[ ! -f "$PROJECT_ROOT/CMakeLists.txt" ]]; then
        log_error "Not in VneScene project root. Please run from project root."
        exit 1
    fi
...
generate_api_docs() {
    ...
    cmake -S "$PROJECT_ROOT" -B "$BUILD_DIR" \
        -DENABLE_DOXYGEN=ON -DVNE_SCENE_LIB_TYPE=shared
    cmake --build "$BUILD_DIR" --target vnescene_doc_doxygen
...
# Main function
main() {
    log_info "Starting VneScene documentation generation..."
...
# Help function
show_help() {
    echo "VneScene Documentation Generator"
    echo ""
